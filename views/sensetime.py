...
class PicHandler(RequestHandler):
    ...
    def Time_consuming_operation(self):
        ...
        if method == 'method1':
            cmd = 'cd {}/defect_classification/Obj_Classify && python predict.py {}/static/picture/'.format(
                project_path, project_path) + pic_name
            val = os.popen(cmd).read()
            result = val.split('类')[1].strip('\n')
            confidence = ''
            return result, pic_id, confidence
        elif method == 'method2':
            cmd = 'cd {}/defect_classification/libtorch-cpp/build && ./classifier ../abc.pt ../label.txt {}/static/picture/'.format(
                project_path, project_path) + pic_name
            val = os.popen(cmd).read()
